...
public class UpdateFacultyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.sendRedirect(response.encodeRedirectURL("UpdateFaculty"));
	}
	
	private void forwardToEditFacultyPage(HttpServletRequest request,
			HttpServletResponse response, Faculty faculty, String message) throws ServletException, IOException {
		
		request.setAttribute("message", message);
		request.setAttribute("faculty", faculty);
		
		request.getRequestDispatcher("WEB-INF/AdminEditFacultyPage.jsp").forward(request, response);
