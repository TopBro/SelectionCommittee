...
public class FacultyRegistrationServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (registrationService.isEnrolleeBlocked(userId)) {
			response.sendRedirect(response.encodeRedirectURL("FacultyRegistration?message=" + BLOCKED_PARAM));
			return;
		}
		
		if (!marksService.isMarksExist(userId)) {
			response.sendRedirect(response.encodeRedirectURL("FacultyRegistration?message=" + MARKS_PARAM));
			return;
		}
		
		if (registrationService.isFacultyClosed(facultyId)) {
			response.sendRedirect(response.encodeRedirectURL("FacultyRegistration?message=" + CLOSED_PARAM));
			return;
		}
		
		if (registrationService.isRegistrationRecordExist(userId, facultyId)) {			
			response.sendRedirect(response.encodeRedirectURL("FacultyRegistration?message=" + WARNING_PARAM));
			return;
		}
		
		registrationService.addRegistrationRecord(userId, facultyId);
		response.sendRedirect(response.encodeRedirectURL("FacultyRegistration?message=" + SUCCESS_PARAM));
	}
