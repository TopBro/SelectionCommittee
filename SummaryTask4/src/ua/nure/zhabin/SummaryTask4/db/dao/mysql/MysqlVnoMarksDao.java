...
import ua.nure.zhabin.SummaryTask4.db.mapper.VnoMarksMapper;
...
public class MysqlVnoMarksDao implements MarksDao<VnoMarks> {
	...
	private static final String UPDATE_MARKS = "Update vno_marks "
			+ "set ukrainian=?, mathematics=?, physics=? "
			+ "where user_id=?";
	...
	public VnoMarks get(long userId, Connection connection) {
		return jdbcTemplate.get(connection, GET_MARKS, new Object[] {userId}, new VnoMarksMapper());
	}
	...
	public void update(VnoMarks marks, Connection connection) {
		jdbcTemplate.update(connection, UPDATE_MARKS, new Object[] {marks.getUkrainian(), 
				marks.getMathematics(), marks.getPhysics(), marks.getUserId()});		
	}
}
