...
import ua.nure.zhabin.SelectionCommittee.locale.SessionLocaleContainer;
...
public class LocaleFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		...
		this.defaultLocale = new Locale(config.getServletContext()
				.getInitParameter(INIT_PARAM_DEFAULT_LOCALE));
	}

	private void initSupportLocales(FilterConfig config) {
		String locales = config.getServletContext().getInitParameter(
				INIT_PARAM_SUPPORT_LOCALES);
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
		...
		localeContainer.saveLocale(httpServletRequest, httpServletResponse,
				locale);
		httpServletRequest.setAttribute("currentLocale", locale.getLanguage()
				.toUpperCase());
	...
	private HttpServletRequestWrapper wrapRequest(
