...
public class MysqlRegistrationDao implements RegistrationDao {
	...
	public List<RegistrationRecord> getAllByUserId(long userId,
		...
		return jdbcTemplate.getAll(connection, GET_REGISTRATIONS_BY_USERS_ID,
				new Object[] { userId }, new RegistrationMapper());
	}

	@Override
	public List<RegistrationRecord> getAllByFacultyId(long facultyId,
			Connection connection) {
		return jdbcTemplate.getAll(connection, GET_REGISTRATIONS_BY_FACULTY_ID,
				new Object[] { facultyId }, new RegistrationMapper());
	}

	@Override
	public RegistrationRecord get(long userId, long facultyId,
			Connection connection) {
		return jdbcTemplate.get(connection, GET_BY_USERS_ID_AND_FACULTY_ID,
				new Object[] { userId, facultyId }, new RegistrationMapper());
	}

	@Override
	public void add(long userId, long facultyId, Connection connection) {
		jdbcTemplate.update(connection, ADD_REGISTRATION_RECORD, new Object[] {
				userId, facultyId, Fields.REGISTRATION_STATUS_WAITING });

	}

	@Override
	public void updateStatus(RegistrationRecord registrationRecord,
			Connection connection) {
		jdbcTemplate.update(
				connection,
				UPDATE_REGISTRATION_STATUS,
				new Object[] { registrationRecord.getStatusId(),
						registrationRecord.getUserId(),
						registrationRecord.getFacultyId() });
