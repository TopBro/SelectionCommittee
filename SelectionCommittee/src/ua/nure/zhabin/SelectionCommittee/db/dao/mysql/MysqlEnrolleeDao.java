...
public class MysqlEnrolleeDao implements EnrolleeDao {
	...
	public Enrollee get(long userId, Connection connection) {
		return jdbcTemplate.get(connection, GET_ENROLLEE,
				new Object[] { userId }, new EnrolleeMapper());
	}

	@Override
	public List<Enrollee> get(Connection connection) {
		return jdbcTemplate.getAll(connection, GET_ALL_ENROLLEES,
				new Object[] {}, new EnrolleeMapper());
	}
	...
	public void add(Enrollee enrollee, Connection connection) {
		jdbcTemplate.update(
				connection,
				ADD_ENROLLEE,
				new Object[] { enrollee.getUserId(), enrollee.getFirstName(),
						enrollee.getMidleName(), enrollee.getLastName(),
						enrollee.getEmail(), enrollee.getCity(),
						enrollee.getRegion(), enrollee.getEducation(),
						enrollee.getStateId() });
	}

	@Override
	public void update(Enrollee enrollee, Connection connection) {
		jdbcTemplate.update(connection, UPDATE_ENROLLEE, new Object[] {
				enrollee.getStateId(), enrollee.getUserId() });
	}
}
