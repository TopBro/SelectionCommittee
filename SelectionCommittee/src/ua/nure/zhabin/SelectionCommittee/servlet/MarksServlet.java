...
public class MarksServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		this.validator = (MarksBeanValidator) getServletContext().getAttribute("MarksBeanValidator");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (marksBean == null) {
			...
			message = "All fields must be filled.<br>Fields for marks may contain numbers only.";
		...
		if (marksService.isMarksExist(user.getId())) {
			...
			response.sendRedirect(response.encodeRedirectURL(Urls.MARKS_SERVLET + "?message=true"));
			return;
		}
	...
	private void forwardToMarksPage(HttpServletRequest request,	HttpServletResponse response, MarksBean marksBean, String message)
	...
	private MarksBean fillMarksBean(HttpServletRequest request) {
		...
		try {
			marksBean.setUkrainian(Integer.parseInt(request.getParameter(Fields.UKRAINIAN)));
			marksBean.setMathematics(Integer.parseInt(request.getParameter(Fields.MATHEMATICS)));
			marksBean.setPhysics(Integer.parseInt(request.getParameter(Fields.PHYSICS)));
			marksBean.setLiterature(Integer.parseInt(request.getParameter(Fields.LITERATURE)));
			marksBean.setHistory(Integer.parseInt(request.getParameter(Fields.HISTORY)));
			marksBean.setEnglish(Integer.parseInt(request.getParameter(Fields.ENGLISH)));
			marksBean.setInformatics(Integer.parseInt(request.getParameter(Fields.INFORMATICS)));
			marksBean.setGeography(Integer.parseInt(request.getParameter(Fields.GEOGRAPHY)));
			marksBean.setBiology(Integer.parseInt(request.getParameter(Fields.BIOLOGY)));
			marksBean.setChemistry(Integer.parseInt(request.getParameter(Fields.CHEMISTRY)));
